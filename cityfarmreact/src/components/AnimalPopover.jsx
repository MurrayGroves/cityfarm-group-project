...
const AnimalPopover = (props) => {
	...
	useEffect(() => {
		if (chosenAnimal.mother) {
			(async () => {
				try {
					const mother = await axios.get(
						`/animals/by_id/${chosenAnimal.mother}`,
						token
					);
		...
		if (chosenAnimal.father) {
			(async () => {
				try {
					const father = await axios.get(
						`/animals/by_id/${chosenAnimal.father}`,
						token
					);
		...
		<div>
			...
			>
				<Typography sx={{ p: 1, whiteSpace: "pre-line" }}>
					{`Type: ${
						chosenAnimal.type
							? chosenAnimal.type.charAt(0).toUpperCase() +
							  chosenAnimal.type.slice(1)
							: "Loading..."
					}`}
					<br />
					{`Father: ${animalFather}`}
