...
const AnimalCreator = (props) => {
    ...
    const fieldTypeSwitch = (key) => {
        ...
        if (newAnimal.fields[field] === undefined) {
            /* initialise field values to enpty strings */
            newAnimal.fields[field] = '';
        }
        /* check the type of the field and display appropriate input method */
        switch (schema._fields[field]._type) {
            case 'java.lang.Boolean':
                return (
                    <FormControl
                        error={newAnimal.fields[field] === '' && schema._fields[field]._required}
                        required={schema._fields[field]._required}
                        sx={{ width: '100%' }}
                    >
                        <FormHelperText style={{ marginLeft: '5px' }}>
                            {schema._fields[field]._required ? 'Required' : 'Not Required'}
                        </FormHelperText>
                        <Select
                        ...
                        >
                            {!schema._fields[field]._required && (
                                <MenuItem value={''}>
                                    <em>Empty</em>
                                </MenuItem>
                            )}
                                    ...
                                    <TableRow>
                                            ...
                                            <Autocomplete
                                                renderOption={(props, option) => {
                                                    return (
                                                        <li {...props} key={option._name}>
                                                            {option._name.charAt(0).toUpperCase() +
                                                                option._name.slice(1)}
                                                        </li>
                                                    );
                                                }}
                                                ...
                                                onChange={(e, v) => {
                                                    if (v) {
                                                        ...
                                                        let tempSchema = props.schemaList
                                                            .filter((schema) => schema._name === v._name)
                                                            .pop();
                                        ...
                                        <TableCell>
                                            <Autocomplete
                                                ...
                                                options={props.animalList
                                                    .filter((animal) => {
                                                        return animal.type === newAnimal.type && animal.sex === 'm';
                                                    })
                                                    .map((animal) => {
                                                        return {
                                                            id: animal._id,
                                        ...
                                        <TableCell>
                                            <Autocomplete
                                                ...
                                                options={props.animalList
                                                    .filter((animal) => {
                                                        return animal.type === newAnimal.type && animal.sex === 'f';
                                                    })
                                                    .map((animal) => {
                                                        return {
                                                            id: animal._id,
                                ...
                                <Table>
                                    <TableHead>
                                        <TableRow>
                                            <TableCell
                                                style={{
                                                    width: `${100 / (Object.keys(schema._fields).length + 1)}%`,
                                                }}
                                            >
                                                Property Name
                                            </TableCell>
                                    ...
                                    <TableBody>
                                        <TableRow>
                                            ...
                                            {Object.keys(schema._fields).map((_, index) => {
                                                return (
                                                    <TableCell key={index}>
                                                        {fieldList ? fieldTypeSwitch(index) : <p>Loading...</p>}
                                                    </TableCell>
                                                );
                                            })}
                                        </TableRow>
