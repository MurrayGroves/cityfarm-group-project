...
const AnimalTable = ({ farms }) => {
    const [animalList, setAnimalList] = useState(
        []
    ); /* The State for the list of animals. The initial state is [] */
    const [searchTerm, setSearchTerm] =
        useState(''); /* The term being searched for in the searchbar */
        ...
        (async () => {
            ...
            try {
                const response = await axios.get(
                    `/animals/by_name/${searchTerm}`,
                    { params: { farm: farm }, ...token }
                );
                setAnimalList(response.data);
            } catch (error) {
                if (error.response.status === 401) {
    ...
    const cols = [
        ...
        },
        {
            ...
            renderCell: (animal) => {
                ...
                return exists ? (
                    <p>
                        {animal.value.type.charAt(0).toUpperCase() +
                            animal.value.type.slice(1)}
                    </p>
                ) : (
                    <p style={{ color: 'red' }}>
                        {animal.value.type.charAt(0).toUpperCase() +
        ...
        },
        {
            ...
            renderCell: (animal) => {
                return animal.value.father ? (
                    <AnimalPopover
                        key={animal.value.father}
                        animalID={animal.value.father}
                    />
                ) : (
                    'Unregistered'
                );
        ...
        },
        {
            ...
            renderCell: (animal) => {
                return animal.value.mother ? (
                    <AnimalPopover
                        key={animal.value.mother}
                        animalID={animal.value.mother}
                    />
                ) : (
                    'Unregistered'
                );
