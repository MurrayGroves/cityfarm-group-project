...
const Schemas = () => {
	const [schemaList, setSchemaList] = useState(
		[]
	); /* The State for the list of enclosures. The initial state is [] */
	const [searchTerm, setSearchTerm] =
		useState(""); /* The term being search for in the searchbar */
	...
	function deleteRow(index) {
		setNewFields(
			newFields.filter((value, thisIndex) => {
				if (thisIndex === index) {
					return false;
				}
		...
		<>
			...
			<TableContainer style={{ marginTop: "20px" }} component={Paper}>
				<div style={{ display: "flex" }}>
					<TextField
						error={newSchemaName === "" && editing && newFields.length > 1}
						required
							...
							>
								<TableCell component="th" scope="row">
									<TextField
										...
										onChange={(e) => {
											setNewFields(
												newFields.map((elem, changeIndex) => {
								...
								<TableCell align="left">
									...
									>
										...
										>
											{Object.entries(classToReadable).map(
												([javaName, className], index) => (
													<MenuItem value={javaName} key={index}>
														{className}
													</MenuItem>
												)
											)}
						...
						<div className="schema-card">
							...
							<TableContainer component={Paper}>
								<Table aria-label="fields table">
											...
											<TableCell width="15%">
												<IconButton
													onClick={async () => {
														try {
															await axios.patch(
																`/schemas/by_name/${schema._name}/hidden`,
																{ hidden: true },
																token
															);
															window.location.reload(false);
									...
									<TableBody>
										{Object.entries(schema._fields).map(
											([name, properties]) => (
												<TableRow
													key={name}
													sx={{
												...
												>
													...
													<TableCell align="left">
														{classToReadable[properties._type]}
													</TableCell>
													<TableCell align="left">
														{properties._required ? "Yes" : "No"}
													</TableCell>
												</TableRow>
											)
										)}
