...
export default ({ msal, setMsal }) => {
	...
	const authHandler = (err, data, msal) => {
		...
		if (
			!data.account.username.endsWith("bristol.ac.uk") &&
			!data.account.username.endsWith("windmillhillcityfarm.org.uk")
		) {
			msal.logoutRedirect({
			...
			{isLoginFailed ? (
				...
				>
					<Alert severity="error">
						Login failed. Please use your city farm email address.
					</Alert>
				</div>
			) : (
				<div></div>
