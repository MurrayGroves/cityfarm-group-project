...
const Calendar = ({ farms }) => {
	...
	const handleAddEvent = () => {
		setAllEvents([
			...allEvents,
			newEvent,
		]); /*Adds the new event to the list of allEvents} */
		setNewEvent({
			title: "",
			allDay: true,
			start: new Date(),
	...
	const eventStyleGetter = (event) => {
		...
		var style = {
			...
			}%, ${theme.SW.main} ${200 / event.farms.length + offset}%, ${
				theme.SW.main
			})`,
			color: "white",
			borderRadius: "5px",
				...
				return (
					<>
						<DateTimePicker
							...
							onChange={(e) => {
								setModifiedEvent({ ...modifiedEvent, start: e.$d });
							}}
							slotProps={{ textField: { fullWidth: true, size: "small" } }}
						/>
						<DateTimePicker
							...
							onChange={(e) => {
								setModifiedEvent({ ...modifiedEvent, end: e.$d });
							}}
							slotProps={{ textField: { fullWidth: true, size: "small" } }}
						/>
					</>
	...
	let todayColour =
		theme.mode === "dark" ? theme.primary.veryDark : theme.primary.light;
		...
		<>
			...
			>
				...
				<div style={{ width: "400px" }}>
						...
						>
							...
							{!modifyEvent ? (
								<div>
									...
									{selectedEvent.farms.includes(farms.WH) ? (
										<p>Windmill Hill</p>
									) : (
										<></>
									)}
									{selectedEvent.farms.includes(farms.HC) ? (
										<p>Hartcliffe</p>
									) : (
										<></>
									)}
									{selectedEvent.farms.includes(farms.SW) ? (
										<p>St Werburghs</p>
									) : (
										<></>
									)}
									{selectedEvent.animals.length !== 0 ? (
										<h3>Animals</h3>
									) : (
										<></>
									)}
									...
									<div>
										...
										<FormGroup>
											<FormControlLabel
												control={
													<Checkbox
														defaultChecked={selectedEvent.farms.includes(
															farms.WH
														)}
														color={farms.HC}
														size="small"
					...
					<Paper
						elevation={3}
						style={{ width: "400px", margin: "0 0 20px 0", padding: "10px" }}
					>
