...
const Calendar = ({ farms }) => {
    ...
    const removeAnimal = (animalID, type) => {
        if (type === 'add') {
            ...
            setModifiedEvent({
                ...modifiedEvent,
                animals: modifiedEvent.animals.filter((animal) => animal !== animalID),
            });
        }
    };
    ...
    const updateVisibleFarms = (selected) => {
        visibleFarms.includes(selected)
            ? setVisibleFarms(visibleFarms.filter((farm) => farm !== selected))
            : setVisibleFarms([...visibleFarms, selected]);
    };

    const eventStyleGetter = (event) => {
        //console.log(event)
        var colour1 = event.farms.includes(farms.WH)
            ? theme.WH.main
            : event.farms.includes(farms.HC)
              ? theme.HC.main
              : theme.SW.main;
        var colour2 = event.farms.includes(farms.HC)
            ? event.farms.includes(farms.WH)
                ? theme.HC.main
                : event.farms.includes(farms.SW)
                  ? theme.SW.main
                  : theme.SW.main
            : theme.SW.main;
        const offset = 0;
        ...
        var style = {
            ...
            backgroundImage: `linear-gradient(135deg, ${colour1}, ${colour1} ${
                100 / event.farms.length - offset
            }%, ${colour2} ${100 / event.farms.length + offset}%, ${colour2} ${
                200 / event.farms.length - offset
            }%, ${theme.SW.main} ${200 / event.farms.length + offset}%, ${theme.SW.main})`,
            color: 'white',
            borderRadius: '5px',
        };
        return {
        ...
        if (type === 'add') {
                ...
                return (
                    <>
                        <DateTimePicker
                            ...
                            onChange={(e) => {
                                setNewEvent({ ...newEvent, start: e.$d });
                            }}
                            slotProps={{
            ...
            if (isShown) {
                return (
                    <>
                        <DateTimePicker
                            value={dayjs(modifiedEvent.start)}
                            placeholder={selectedEvent.start}
                            onChange={(e) => {
                                setModifiedEvent({
                                    ...modifiedEvent,
                                    start: e.$d,
                        ...
                        />
                        <DateTimePicker
                            value={dayjs(modifiedEvent.end)}
                            placeholder={selectedEvent.end}
                            onChange={(e) => {
                                setModifiedEvent({
                                    ...modifiedEvent,
                                    end: e.$d,
    ...
    const onRangeChange = useCallback(async (range) => {
        if (range.start !== undefined) {
            //month or agenda case start and end are the times displayed on the calendar
            try {
            ...
            if (range[1] !== undefined) {
                //week case has an array of 7 times
                try {
                ...
                // day case has a single element of the start time
                try {
                    const start = range[0];
                    const end = start;
    ...
    return (
        <>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Paper
                    elevation={3}
                    style={{ width: '400px', height: '48px', marginTop: '21.44px', padding: '6px 0 0 10px' }}
                >
                    <FormGroup row>
                        <FormControlLabel
                            control={<Checkbox defaultChecked color={farms.WH} size="small" />}
                            label="Windmill Hill"
                            onChange={() => updateVisibleFarms(farms.WH)}
                        />
                        <FormControlLabel
                            control={<Checkbox defaultChecked color={farms.HC} size="small" />}
                            label="Hartcliffe"
                            onChange={() => updateVisibleFarms(farms.HC)}
                        />
                        <FormControlLabel
                            control={<Checkbox defaultChecked color={farms.SW} size="small" />}
                            label="St Werburghs"
                            style={{ margin: '0' }}
                            onChange={() => updateVisibleFarms(farms.SW)}
                        />
                    </FormGroup>
                </Paper>
            </div>
                ...
                <div style={{ width: '400px' }}>
                            ...
                            >
                                ...
                                <IconButton
                                    ...
                                    onClick={() => {
                                        ...
                                        setSelectedEvent('No event selected');
                                    }}
                                >
                                    <img
                                        src={theme.mode === 'dark' ? CloseIconDark : CloseIconLight}
                                        alt="close button"
                                    />
                                </IconButton>
                            </div>
                            {!modifyEvent ? (
                                ...
                                <div className="modifyEvent">
                                    ...
                                    <div style={{ marginTop: '10px' }}>
                                        <FormControlLabel
                                            control={<Checkbox defaultChecked={selectedEvent.allDay} size="small" />}
                                            label="All Day"
                                            onChange={(e) => {
                                                changeAllDay(!modifiedEvent.allDay, 'modify');
                                            }}
                                        />
                                    ...
                                    <div>
                                        ...
                                        <FormGroup>
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        defaultChecked={selectedEvent.farms.includes(farms.WH)}
                                                        color={farms.HC}
                                                        size="small"
                                                    />
                                                }
                                                ...
                                                onChange={() =>
                                                    setModifiedEvent({
                                                        ...modifiedEvent,
                                                        farms: modifiedEvent.farms.includes(farms.WH)
                                                            ? modifiedEvent.farms.filter((farm) => farm !== farms.WH)
                                                            : modifiedEvent.farms.concat(farms.WH),
                                                    })
                                                }
                                            />
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        defaultChecked={selectedEvent.farms.includes(farms.HC)}
                                                        color={farms.HC}
                                                        size="small"
                                                    />
                                                }
                                                ...
                                                onChange={() =>
                                                    setModifiedEvent({
                                                        ...modifiedEvent,
                                                        farms: modifiedEvent.farms.includes(farms.HC)
                                                            ? modifiedEvent.farms.filter((farm) => farm !== farms.HC)
                                                            : modifiedEvent.farms.concat(farms.HC),
                                                    })
                                                }
                                            />
                                            <FormControlLabel
                                                control={
                                                    <Checkbox
                                                        defaultChecked={selectedEvent.farms.includes(farms.SW)}
                                                        color={farms.SW}
                                                        size="small"
                                                    />
                                                }
                                                ...
                                                onChange={() =>
                                                    setModifiedEvent({
                                                        ...modifiedEvent,
                                                        farms: modifiedEvent.farms.includes(farms.SW)
                                                            ? modifiedEvent.farms.filter((farm) => farm !== farms.SW)
                                                            : modifiedEvent.farms.concat(farms.SW),
                                                    })
                                                }
                                            />
                                        </FormGroup>
                                    ...
                                    {/* Apply changes to do with associating animals here */}
                                    <div>
                                        <h3>Enclosures</h3>
                                        ...
                                        <Button variant="outlined">Add Enclosure</Button>{' '}
                    ...
                    >
                        ...
                        <div style={{ marginTop: '10px' }}>
                            ...
                            />
                            <Button variant="outlined" style={{ float: 'right' }} onClick={() => handleAddEvent()}>
                                Create
                            </Button>
                        </div>
                        ...
                        <div style={{ marginTop: '10px' }}>
                            ...
                            <FormGroup>
                                <FormControlLabel
                                    control={<Checkbox color={farms.WH} size="small" />}
                                    label="Windmill Hill"
                                    onChange={() =>
                                        setNewEvent({
                                            ...newEvent,
                                            farms: newEvent.farms.includes(farms.WH)
                                                ? newEvent.farms.filter((farm) => farm !== farms.WH)
                                                : newEvent.farms.concat(farms.WH),
                                        })
                                    }
                                />
                                <FormControlLabel
                                    control={<Checkbox color={farms.HC} size="small" />}
                                    label="Hartcliffe"
                                    onChange={() =>
                                        setNewEvent({
                                            ...newEvent,
                                            farms: newEvent.farms.includes(farms.HC)
                                                ? newEvent.farms.filter((farm) => farm !== farms.HC)
                                                : newEvent.farms.concat(farms.HC),
                                        })
                                    }
                                />
                                <FormControlLabel
                                    control={<Checkbox color={farms.SW} size="small" />}
                                    label="St Werburghs"
                                    onChange={() =>
                                        setNewEvent({
                                            ...newEvent,
                                            farms: newEvent.farms.includes(farms.SW)
                                                ? newEvent.farms.filter((farm) => farm !== farms.SW)
                                                : newEvent.farms.concat(farms.SW),
                                        })
                                    }
                                />
                            </FormGroup>
